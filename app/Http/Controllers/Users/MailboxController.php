...
class MailboxController extends UserController
{
    ...
    {
        $email_list = $this->emailRepository->getAll()->with('sender')->where('to', $this->user->id)->where('delete_receiver', 0)->orderBy('id', 'desc')->get();
        $sent_email_list = $this->emailRepository->getAll()->with('receiver')->where('from', $this->user->id)->where('delete_sender', 0)->orderBy('id', 'desc')->get();
        ...
        $users_list = $this->userRepository->getAll()->get()
            ->map(function ($user) {
                return [
        ...
        $email_templates = $this->emailTemplateRepository->getAll()->get()
            ->map(function ($email) {
                return [
                    'id' => $email->id,
                    'text' => '('.$email->id.') '.$email->title,
                ];
            })
            ->prepend(['id'=> 0, 'text'=>trans('dashboard.select_email_template')])
            ->values();

        $have_email_template = count($email_templates);
        ...
        if (!empty($request->recipients)) {
            foreach ($request->recipients as $item) {
                if ($item != "0" && $item != "") {
                    ...
                    $email->save();
                    broadcast(new EmailSentEvent($email->id))->toOthers();
    ...
    public function getSent()
    {
        ...
        $sent = $this->emailRepository->getAll()->where('from', $user->id)
    ...
    {
        $user = $this->userRepository->getUser();
        $received_list = $this->emailRepository->getAll()->where('to', $user->id)
            ->where('delete_receiver', 0)
