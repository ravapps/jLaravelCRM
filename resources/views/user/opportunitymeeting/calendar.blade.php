    ...
    <div class="calendar">
        <div class="page-header clearfix">
            <div class="pull-right">
                @if($user_data->hasAccess(['meetings.read']) || $user_data->hasAccess(['opportunities.read'])  || $user_data->inRole('admin'))
                    <a href="{{ url($type.'/'.$opportunity->id) }}" class="btn btn-success">
    ...
    <script>
        $(document).ready(function() {
            $('#calendar').fullCalendar({
                ...
                "eventSources": [
                    {
                        url: "{{url('opportunitymeeting/'.$opportunity->id.'/calendarData')}}",
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}'
                        },
